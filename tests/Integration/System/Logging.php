...
class CampaignTest extends TestCase
{
     ...
     * @test
     */
    public function log_access_get_no_user()
    {
        // Prepare
        (new CreateLogs())->handle();
        
        // Execute
        Logger::forceMethod('GET');
        Logger::forceUrl('http://greekhouse.dev/path');
        Logger::setModule('foo');
        Logger::logAccess('bar');
        
        // Assert
        $this->assertDatabaseHas('logs', ['message' => '[GET] (#foo) bar http://greekhouse.dev/path'], 'logs');
    }
    ...
    public function can_read_campaign()
    {
        ...
        $response->assertSeeText($campaign->decorator->getFullName());
        ...
        $response->assertSee(route('system::image', [$campaign->product_colors->first()->image_id]));
        $response->assertSeeText($campaign->contact_first_name);
        $response->assertSeeText($campaign->contact_last_name);
        ...
        $response->assertSeeText($campaign->address_name);
    ...
    public function can_submit_update_actors()
    {
        ...
        $response->assertStatus(200);
        $response->assertSeeText('Order Information Saved');
        $response->assertSeeText($newUser->getFullName());
        $response->assertSeeText($newDesigner->getFullName());
        $response->assertSeeText($newDecorator->getFullName());
    }
    ...
    public function can_read_update_products()
    {
        ...
        // Assert
        $response->assertStatus(200);
        $response->assertSee((string)$campaign->product_colors->first()->product_id);
        $response->assertSee((string)$campaign->product_colors->first()->id);
    }
    
    /**
     * @test
     */
    public function can_submit_update_products_only_color()
    {
        // Prepare
        (new \TestingSeeder())->run();
        (new \ProductSeeder())->run();
        ...
        $campaign = CampaignGenerator::create('delivered')->withDesigner($designer)->withDesign()->withOwner($user)->withDecorator($decorator)->campaign();
        $support = UserGenerator::create('support')->user();
        $currentColor = $campaign->product_colors->first()->id;
        $newColor = product_color_repository()->getByProductId($campaign->product_colors->first()->product_idid, $currentColor)->first();
        $this->be($support);
        $response = $this->get('/admin/campaign/read/' . $campaign->id);
        $response->assertDontSeeText($newColor->name);
        
        // Execute
        $response = $this->post('/admin/campaign/update-products/' . $campaign->id, [
            'product_id' => $campaign->product_colors->first()->product_id,
        ...
        $response->assertStatus(200);
        $response->assertSeeText('Order Information Saved');
        $response->assertSeeText($newColor->name);
    }
    ...
    public function can_get_product_colors()
    {
        ...
        $support = UserGenerator::create('support')->user();
        $currentColor = $campaign->product_colors->first()->id;
        $newColor = product_color_repository()->getByProductId($campaign->product_colors->first()->product_id, $currentColor)->first();
        $this->be($support);
    ...
    public function can_read_update_contact()
    {
        ...
        $response->assertSee($campaign->contact_school);
        $response->assertSee($campaign->contact_chapter);
    }
    ...
    public function can_read_update_shipping()
    {
        ...
        // Assert
        $response->assertStatus(200);
        $response->assertSee($campaign->address_name);
